...
1. itens em que regras e LLM DIVERGEM na disciplina/tópico;
...
WITH r AS (SELECT item_id, topico, subtopico, confianca
     ...
     l AS (SELECT item_id, topico, subtopico, confianca
           FROM classificacoes WHERE metodo = 'llm'),
...
SELECT i.id, i.numero, c.nome, p.cargo, i.texto,
       ...
       l.topico, l.subtopico, l.confianca
FROM itens i
...
def _mostrar(item: tuple) -> None:
    (item_id, numero, concurso, cargo, texto, motivador,
     r_top, r_sub, r_conf, l_top, l_sub, l_conf) = item
    print("\n" + "=" * 78)
    ...
    print(f"  2) llm:    {l_top or '—'} > {l_sub or '—'}  (conf. {l_conf})")
...
def revisar(con) -> None:
    ...
    print("Comandos: 1=aceitar regras | 2=aceitar llm | d=digitar | p=pular | q=sair")
    for item in fila:
        _mostrar(item)
        escolha = input("\n> ").strip().lower()
        if escolha == "q":
    ...
    if args.listar:
        ...
        for item in fila[:30]:
            print(f"  {item[2]:22s} item {item[1]:3d}  "
                  f"regras={item[6] or '—'}  llm={item[9] or '—'}")
        if len(fila) > 30:
